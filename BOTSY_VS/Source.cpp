...
/*
    ...
    server.on("/", HTTP_GET, [](AsyncWebServerRequest* request) {
        request->send(200, "text/plain", SD.open("index.html", FILE_READ).readString());
        });
    ...
    server.onNotFound("/", HTTP_GET, [](AsyncWebServerRequest* request) {
        request->send(SD, "/errors/404.html");
        });
...
void handle_Json() {
    ...
    for (int i = 0; i < N; i++) {
        doc["SITES"][i]["SSID"] = wifi_sort[i].ssid;
        doc["SITES"][i]["RSSI"] = wifi_sort[i].strength;
        if (wifi_sort[i].secure == AUTH_OPEN)           doc["SITES"][i]["Secure"] = "Открытая сеть";
        else if (wifi_sort[i].secure == ENC_TYPE_WEP)   doc["SITES"][i]["Secure"] = "WEP";
        else if (wifi_sort[i].secure == ENC_TYPE_TKIP)  doc["SITES"][i]["Secure"] = "WPA / PSK";
        else if (wifi_sort[i].secure == ENC_TYPE_CCMP)  doc["SITES"][i]["Secure"] = "WPA2 / PSK";
        else if (wifi_sort[i].secure == ENC_TYPE_NONE)  doc["SITES"][i]["Secure"] = "Открытая сеть";
        else if (wifi_sort[i].secure == ENC_TYPE_AUTO)  doc["SITES"][i]["Secure"] = "WPA / WPA2 / PSK";
        else                                            doc["SITES"][i]["Secure"] = "Неизвестно";
        doc["SITES"][i]["MAC"] = wifi_sort[i].MAC;
    }
    char buffer[10240];
    server.send(200, "application/json", (String)serializeJson(doc, buffer));
}
